...
async function seedAllProjects(force = false) {
  ...
  try {
    ...
    await Promise.all(files.map(file => require(file)(force))); // eslint-disable-line global-require,import/no-dynamic-require,max-len
  } catch (e) {
    console.error(e);
  }
}
